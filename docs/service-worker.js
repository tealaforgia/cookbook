...
self.addEventListener('fetch', (event) => {
    ...
    event.respondWith((async () => {
        // 1. TENTATIVO DIRETTO: C'è nella cache esattamente così?
        const cachedResponse = await caches.match(event.request);
        if (cachedResponse) {
            return cachedResponse;
        }

        // 2. REWRITE PREDITTIVO (Anti-Console Errors)
        // Prima di provare la rete e fallire, controlliamo se è un "path fantasma".
        // Questo risolve sia il problema dei log rossi sia sitemap/favicon.
        
        let fixedUrl = null;

        // Caso A: Risorse dentro /assets/ (gestisce i percorsi relativi di MkDocs)
        if (url.pathname.includes('/assets/')) {
            // Prende tutto quello che c'è dopo l'ultimo '/assets/' e lo ancora alla root
            const assetPath = '/assets/' + url.pathname.split('/assets/').pop();
            fixedUrl = new URL(assetPath, BASE_URL);
        }
        
        // Caso B: File di root specifici (sitemap, favicon, manifest)
        // Se l'URL finisce con uno di questi file MA non è nella root (es: /pagina/sitemap.xml)
        ...
        // Se abbiamo ipotizzato un URL corretto, controlliamo SE è in cache.
        if (fixedUrl) {
            const cachedFixedAsset = await caches.match(fixedUrl);
            if (cachedFixedAsset) {
                // Trovato! Restituiamo questo ed evitiamo il fetch fallito.
                return cachedFixedAsset;
            }
        }

        // 3. RETE (Se non era in cache né originale né fixato, proviamo internet)
        try {
            ...
            if (event.request.destination === 'image' || url.pathname.match(/\.(jpg|png|svg|avif|webp|gif|mp4|webm|pdf|mp3|ogg|wav|jxl)$/i)) {
                ...
                const offlinePhrases = [
                    ...
                    "La connessione è bloccata nel 1986. Sic mundus creatus est.",
                    "Say my name. Sei Offline. You're goddamn right.",
                    "Il segnale è finito nel Sottosopra.",
                    "Manca il legame covalente tra te e il server.",
                    "Hanno suonato al server, ma lui ha fatto finta di non esserci.",
                    "Questa pagina si è disassociata dalla rete.",
                    ...
                    "Useless lesbian drama: il modem non parla col PC.",
                    ...
                    "Prima regola del Fight Club: sei offline.",
                    ...
                    "Sono velocità. Pura velocità. (No, scherzo, sono fermo).",
                    "Zero Assoluto (-273.15°C): Il server è congelato."
                ];
                
                // Pesca una frase a caso
                ...
                <svg width="100%" height="100%" viewBox="0 0 400 120" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <g transform="translate(164, 5) scale(0.5)"> 
                    ...
                    <text x="50%" y="92" font-family="sans-serif" text-anchor="middle">
                        <tspan x="50%" dy="0" font-size="11" class="text-title">${randomTitle}</tspan>
                        <tspan x="50%" dy="14" font-family="monospace" font-size="9" class="text-file">${filename}</tspan>
                    </text>
                </svg>`;
